def count_recover_price_hardnum(order_books, recover_price):
    ...
    return float(re_total_volumn) / float(total_volumn), total_volumn
...
def test_count_recover_price_hardnum():
    ...
    result, total_volumn = count_recover_price_hardnum(Ask, 51.15)
    if result == float(275000 + 932000 + 222500 + 90500) / float(total_volumn):
        print("ok",result)
    else:
        print("fail")
    ###测试牛证回收价 50.8, 牛证是看下跌，下跌看买方Bid
    result, total_volumn = count_recover_price_hardnum(Bid, 50.8)
